...
db.execute("CREATE TABLE IF NOT EXISTS accounts (name TEXT PRIMARY KEY NOT NULL, balance INTEGER NOT NULL)")  # noqa
...
class Account(object):
    ...
    def __init__(self, name: str, opening_balance: int = 0):
        cursor = db.execute("SELECT name, balance FROM accounts WHERE (name = ?)",  # noqa
                            (name,))
        row = cursor.fetchone()
    ...
    def _save_update(self, amount):
        ...
        deposit_time, zone = Account._current_time()  # <-- unpack the returned tuple  # noqa
        picked_zone = pickle.dumps(zone)

        db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)", (new_balance, self.name))  # noqa
        db.execute("INSERT INTO history VALUES(?, ?, ?, ?)", (deposit_time, self.name, amount, picked_zone))  # noqa  # To fix this find out why this is having a problem. Look at the history. 
        db.commit()
    ...
    def deposit(self, amount: int) -> float:
        if amount > 0.0:
            ...
            # db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)",  # noqa
