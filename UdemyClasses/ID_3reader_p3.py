# Read ID3 tags from a file. Ned Batchelder,
# http://nedbatchelder.com/code/modules/id3reader.html This code is in the public domain.  # noqa
import struct
import sys
import zlib
...
_genres = [
    ...
    'Gothic', 'Darkwave', 'Techno - Industrial', 'Electronic', 'Pop - Folk', 'Eurodance', 'Dream', 'Southern Rock',  # noqa
    'Comedy', 'Cult', 'Gangsta Rap',
    # 60-79
    'Top 40', 'Christian Rap', 'Pop / Funk', 'Jungle', 'Native American', 'Cabaret', 'New Wave', 'Psychedelic',  # noqa
    'Rave', 'Showtunes', 'Trailer', 'Lo - Fi', 'Tribal', 'Acid Punk', 'Acid Jazz', 'Polka', 'Retro', 'Musical',  # noqa
    'Rock & Roll', 'Hard Rock',
    # 80-99
    'Folk', 'Folk / Rock', 'National Folk', 'Swing', 'Fast - Fusion', 'Bebob', 'Latin', 'Revival', 'Celtic',  # noqa
    'Bluegrass', 'Avantgarde', 'Gothic Rock', 'Progressive Rock', 'Psychedelic Rock', 'Symphonic Rock',  # noqa
    'Slow Rock', 'Big Band', 'Chorus', 'Easy Listening', 'Acoustic',
    # 100-119
    'Humour', 'Speech', 'Chanson', 'Opera', 'Chamber Music', 'Sonata', 'Symphony', 'Booty Bass', 'Primus',  # noqa
    'Porn Groove', 'Satire', 'Slow Jam', 'Club', 'Tango', 'Samba', 'Folklore', 'Ballad', 'Power Ballad',  # noqa
    'Rhythmic Soul', 'Freestyle',
    # 120-139
    'Duet', 'Punk Rock', 'Drum Solo', 'A Cappella', 'Euro - House', 'Dance Hall', 'Goa', 'Drum & Bass',  # noqa
    'Club - House', 'Hardcore', 'Terror', 'Indie', 'BritPop', 'Negerpunk', 'Polsk Punk', 'Beat',  # noqa
    'Christian Gangsta Rap', 'Heavy Metal', 'Black Metal', 'Crossover',
    # 140-147
    'Contemporary Christian', 'Christian Rock', 'Merengue', 'Salsa', 'Thrash Metal', 'Anime', 'JPop', 'Synthpop'  # noqa
]
...
class _Frame:
    ...
    def __init__(self):
        ...
        self.b_encrypted = False
        self.b_in_group = False
    ...
    def _interpret(self) -> None:
        """Examine self.raw_data and create a self.value from it."""
        conditions = {
            self.id[0] == 'T': self._interpret_text,
            self.id[0] == 'W': self._interpret_url,
            self.id == 'CDM': self._interpret_cdm,
            self.id in _simpleDataMapping['comment']: self._interpret_comment
        }
    ...
    def _interpret_text(self):
        encoding = self.raw_data[0]
        if 0 <= encoding < len(_encodings):
            value = self.raw_data[1:].decode(_encodings[encoding])
        else:
    ...
    def _interpret_url(self):
        if isinstance(self.raw_data, str):
            self.value = self.raw_data.strip('\0')
        else:
            self.value = self.raw_data.strip(b'\0')
        if self.id == 'WXXX':
            if isinstance(self.raw_data, str):
    ...
    def _interpret_cdm(self):
        if self.raw_data[0] == 'z':
            self.raw_data = zlib.decompress(self.raw_data[5:])
        else:
            raise Id3Error('Unknown CDM compression: {:02x}'.format(self.raw_data[0]))  # noqa

    def _interpret_comment(self):
        s = str(self.raw_data)
        pos = 0
        count = 0
        ...
        if self.b_compressed:
            # Decompress the compressed data.
            self.raw_data = zlib.decompress(self.raw_data)

        # print("in _interpret, id is {}, id[0] is {}".format(self.id, self.id[0]))  # noqa
        if self.id[0] == 'T':
            # Text fields start with T
            # encoding = ord(self.raw_data[0])
            encoding = self.raw_data[0]
            ...
            print("in _interpret, self.raw_data is {}".format(self.raw_data))
            if isinstance(self.raw_data, str):
                self.value = self.raw_data.strip('\0')
            else:
                self.value = self.raw_data.strip(b'\0')
...
class Reader:
    ...
    def _read_bytes(self, num, desc=''):
        ...
        if num > self.bytesLeft:
            ...
            raise Id3Error('Long read {}: {} {}'.format(desc, num, self.bytesLeft))  # noqa
        bytes_read = self.file.read(num)
        self.bytesLeft -= num

        if len(bytes_read) < num:
            # if _t: _trace("short read with %d left, %d total" % (self.bytesLeft, self.header.size))  # noqa
            # if _c: _coverage('short!')
            raise Id3Error('Short read {}: {} {}'.format(desc, len(bytes_read), num))  # noqa
    ...
    def _add_v1_frame(self, f_id, rawdata):
        # print("in _add_v1_frame, f_id is {}\n\trawdata is {}".format(f_id, rawdata))  # noqa
        if f_id == 'v1genre':
            ...
            if 0 <= encoding < len(_encodings):
                ...
                # This is a bit iffy.
                # It may seem more reasonable to try iso8859-1 first, but that fails the  # noqa
                # ID3v1 test 272 in the test pack at http://id3.org/Developer%20Information.  # noqa
                #
                # This may be a bad decision, in which case swap the two decode lines.  # noqa
    ...
    def _read_id3(self):
        ...
        if self.header.majorVersion == 2:
            ...
            raise Id3Error("Unsupported major version: {}".format(self.header.majorVersion))  # noqa
    ...
    def _read_frame_rev2(self):
        ...
        frame.raw_data = self._read_bytes(frame.size, 'rev2data')
        return frame
    ...
    def _read_frame_rev3(self):
        ...
        if frame.bCompressed:
            frame.decompressedSize = self._get_integer(self._read_bytes(4, 'decompsize'))  # noqa
            cb_data -= 4
        ...
        frame.raw_data = self._read_bytes(cb_data, 'rev3data')
    ...
    def _read_frame_rev4(self):
        ...
        if frame.flags & 0x0001:
            frame.datalen = self._get_sync_safe_int(self._read_bytes(4, 'datalen'))  # noqa
            cb_data -= 4
    ...
    def get_raw_data(self, f_id):
        """This line of code creates a function called get_raw_data
        which has two arguments self, which refers to the instance of the
        class. And f_id, which is being used as a identifier.  """
        if f_id in self.frames:
            return self.frames[f_id].raw_data
        return None

    def dump(self):
        """This line of code creates a function called dump"""
        import pprint
        print("Header:")
        print(self.header)
        print("Frames:")
        for fr in self.allFrames:
            if len(fr.raw_data) > 30:
                fr.raw_data = fr.raw_data[:30]
        pprint.pprint(self.allFrames)
        for fr in self.allFrames:
            if hasattr(fr, 'value'):
                print(f'{fr.id}: {_safestr(fr.value)}')
            else:
                print(f'{fr.id}= {_safestr(fr.raw_data)}')
        for label in _simpleDataMapping.keys():
            v = self.get_value(label)
            if v:
                print(f'Label {label}: {_safestr(v)}')

    @staticmethod
    def dump_coverage():
        """This line creates a function called dump_coverage"""
        feats = sorted(_features.keys())
        for feat in feats:
            print("Feature {:12}: f{feat, _features[feat]}")
...
# History:
# 20040104: Created.
# 20040105: Two bugs: didn't read v1 properly, and didn't like empty strings in values.  # noqa
#
...
#           Be more generous about finding album and performer names in the tag.  # noqa
#
